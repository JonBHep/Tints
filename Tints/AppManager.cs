...
namespace Jbh
{
    internal static class AppManager
    {
        ...
        // This file edited 23-04-2022 to accommodate data held in subfolders of the standard data folder
        ...
        */
        ...
        internal static string AppName
        {
            get
            {
                ...
                if (apNam.EndsWith(".vshost", StringComparison.InvariantCultureIgnoreCase))
                {
                    apNam = apNam[0..^7];
                } // in case running in IDE
        ...
        private static string UsualDataPath
        {
            ...
            get
            {
                ...
                if (di.RootDirectory.Name.StartsWith("C", StringComparison.InvariantCultureIgnoreCase))
                {
                    ...
                    appDataPath = Path.Combine(di.RootDirectory.FullName, "Jbh.Original", "Jbh.Info", "AppData"
                        , AppName);
                }
                else
                {
                    ...
                    appDataPath = Path.Combine(di.RootDirectory.FullName, "Jbh.Portable", "Jbh.Info", "AppData"
                        , AppName);
                }
                ...
                System.Windows.MessageBox.Show(
                    $"{runLocationMessage}\n\nFailed to locate the AppData path: {appDataPath}", AppName
        ...
        {
            var extn = Path.GetExtension(dataFileSpec);
            var folder = Path.GetDirectoryName(dataFileSpec);
            if (folder is { })
            {
                FileInfo fi = new(dataFileSpec);
                var timeStamp = $"{fi.LastWriteTimeUtc:yyyy-MM-dd-HH-mm-ss}";
                var backupPath = Path.Combine(folder, $"Backup-{timeStamp}{extn}");
                ...
                {
                    File.Copy(dataFileSpec, backupPath);
                }
            }
        }
        
        private static void PurgeBackups(string targetFolder, string fileExtension, int minimumDaysToKeep
            , int minimumFilesToKeep)
        {
            if (!fileExtension.StartsWith(".", StringComparison.InvariantCultureIgnoreCase))
            {
                fileExtension = "." + fileExtension;
            }
            
            List<string> previousFiles = Directory.GetFiles(targetFolder, "Backup-????-??-??-??-??-??" + fileExtension
                , SearchOption.TopDirectoryOnly).ToList();
            if (previousFiles.Count <= minimumFilesToKeep)
            {
                return;
            }
            ...
            do
            {
                ...
                foreach (string s in previousFiles)
                {
                    ...
                    {
                        overAged++;
                        if (fi.LastWriteTimeUtc < oldestDate)
                        {
                            oldestDate = fi.LastWriteTimeUtc;
                            oldest = s;
                        }
                    }
                }
        ...
        {
            // For use when data is held in the DataPath folder (as is usual)
            PurgeBackups(DataPath,fileExtension, minimumDaysToKeep, minimumFilesToKeep);
        }
        
        internal static void PurgeOldBackups(string appDataSubFolder, string fileExtension, int minimumDaysToKeep
            , int minimumFilesToKeep)
        {
            // For use when data sets are held in subfolders of DataPath e.g. FamilyTrees app
            string path = Path.Combine(DataPath, appDataSubFolder);
            PurgeBackups(path,fileExtension, minimumDaysToKeep, minimumFilesToKeep);
        }
